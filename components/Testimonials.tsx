...
import { Star, TrendingUp, X, ZoomIn } from "lucide-react";
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-[#6EE800] text-xs font-bold tracking-widest uppercase">
            Transformation Story — RyZeey (@Eisblokk)
          </span>
        </div>
            ...
            >
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors duration-200 flex items-center justify-center">
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-black/70 rounded-full p-2">
                    <ZoomIn className="w-4 h-4 text-white" />
                  </div>
                </div>
                {/* Rank badge */}
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent px-2 pt-4 pb-1.5 pointer-events-none">
                  <span className="text-[#6EE800] text-[10px] font-bold leading-none block truncate">{r.rank}</span>
                </div>
                {/* Step badge */}
                {r.badge && (
                  <div className="absolute top-2 left-2 bg-[#6EE800] rounded-full px-1.5 py-0.5 pointer-events-none">
                    <span className="text-black text-[9px] font-black">{r.badge}</span>
                  </div>
                )}
              </button>
              ...
              <div className="p-3 flex flex-col gap-1.5 flex-1">
                <div className="flex items-center gap-1.5">
                  <div className="w-5 h-5 rounded-full bg-[#6EE800]/15 border border-[#6EE800]/30 flex items-center justify-center text-[#6EE800] text-[8px] font-black flex-shrink-0">
                    {r.avatar}
                  </div>
                  <span className="text-white font-bold text-xs truncate">{r.name}</span>
                  <div className="flex gap-0.5 ml-auto flex-shrink-0">
                    {[...Array(5)].map((_, j) => (
                      <Star key={j} className="w-2 h-2 text-[#6EE800] fill-[#6EE800]" />
                    ))}
                  </div>
                </div>
                ...
                <span className="text-gray-600 text-[10px] mt-auto">{r.date}</span>
              </div>
            </motion.div>
          ))}
        </div>
